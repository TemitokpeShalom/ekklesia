    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">{{ members.length }} membre{{ members.length > 1 ? 's' : '' }}</h1>
            </div>
        </template>
        ...
        <div class="space-y-8">
            ...
            <div v-if="members.length === 0" class="glass-panel flex flex-col items-center rounded-3xl px-8 py-14 text-center animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <p class="mt-1 text-sm text-white/40">Ajoute un premier membre pour commencer.</p>
            </div>
            ...
            <div v-else class="space-y-3">
                ...
                >
                    <img
                        ...
                        class="h-10 w-10 flex-shrink-0 rounded-full object-cover border border-white/15"
                    />
                    <span
                        v-else
                        class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-white/5 text-xs font-semibold text-white/60"
                    >
                        {{ initials(member) }}
                    </span>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span
                                ...
                                :class="member.status === 'active' ? 'bg-forest/15 text-forest' : 'bg-white/10 text-white/50'"
                            >
