  ...
  <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
    ...
    <div class="max-w-2xl mx-auto space-y-10">
      <form @submit.prevent="submit" class="glass-panel rounded-3xl p-6 md:p-7 animate-[fadeInUp_0.55s_ease-out_both]">
        <div class="mb-4">
          <label class="block text-xs font-semibold text-gold-soft/80 uppercase tracking-wide mb-1.5">Catégorie</label>
          ...
          <p v-if="form.errors.category" class="text-sm text-rose-400 mt-1">{{ form.errors.category }}</p>
        </div>

        <div class="mb-4">
          <label class="block text-xs font-semibold text-gold-soft/80 uppercase tracking-wide mb-1.5">Description</label>
          <textarea v-model="form.message" rows="4" maxlength="5000"
            class="w-full bg-white/5 border border-white/15 text-white placeholder-white/30 rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60"></textarea>
          <p v-if="form.errors.message" class="text-sm text-rose-400 mt-1">{{ form.errors.message }}</p>
        </div>

        <label class="flex items-center gap-2 text-sm text-white/70 mb-5">
          <input v-model="form.is_anonymous" type="checkbox" class="rounded border-white/25 bg-white/5 text-gold focus:ring-gold/50" />
          Envoyer anonymement
        </label>
      ...
      <div>
        <h3 class="font-serif text-xl text-white mb-4">
        ...
        <div class="space-y-4">
          <div v-for="s in localSignalements" :key="s.id" class="glass-panel rounded-2xl p-5">
            <div class="flex items-center justify-between mb-2">
              <span class="text-xs uppercase tracking-widest text-gold-soft/90 font-semibold">{{ s.category }}</span>
              <span class="text-xs text-white/40">{{ formatDate(s.created_at) }}</span>
            </div>
            <p class="text-sm text-white/85 mb-3">{{ s.message }}</p>
            <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-white/45">
              ...
              <select v-if="canManage" :value="s.status" @change="onStatusChange(s, $event.target.value)"
                class="bg-white/5 border border-white/15 text-white rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-gold/50">
                <option v-for="(label, value) in STATUS_LABELS" :key="value" :value="value" class="bg-night text-white">{{ label }}</option>
              </select>
              <span v-else class="inline-flex items-center gap-1.5 font-medium text-white/80">
