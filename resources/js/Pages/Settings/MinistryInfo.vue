    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">Informations du ministère</h1>
        ...
        <div class="max-w-lg mx-auto">
            <form @submit.prevent="submit" class="glass-panel rounded-3xl p-6 md:p-7 animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <input id="name" v-model="form.name" type="text" required
                    class="w-full bg-white/5 border border-white/15 text-white rounded-xl px-3.5 py-2.5 mb-1 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60" />
                <p v-if="form.errors.name" class="text-sm text-rose-400 mb-3">{{ form.errors.name }}</p>

                <div class="grid grid-cols-2 gap-3 mt-4">
                    <div>
                        <label class="block text-sm font-medium text-white/80 mb-1" for="acronym">Sigle</label>
                        <input id="acronym" v-model="form.acronym" type="text"
                            class="w-full bg-white/5 border border-white/15 text-white rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60" />
                ...
                <div class="grid grid-cols-2 gap-3 mt-4">
                    <div>
                        <label class="block text-sm font-medium text-white/80 mb-1" for="phone">Téléphone</label>
                        <input id="phone" v-model="form.phone" type="text"
                            class="w-full bg-white/5 border border-white/15 text-white rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60" />
                        <p v-if="form.errors.phone" class="mt-1 text-sm text-rose-400">{{ form.errors.phone }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-white/80 mb-1" for="email">E-mail</label>
                ...
                <label class="block text-sm font-medium text-white/80 mb-1 mt-4" for="website">Site web</label>
                ...
                <label class="block text-sm font-medium text-white/80 mb-1 mt-4">Logo</label>
                <div class="flex items-center gap-3">
                    <img v-if="logoPreview" :src="logoPreview" class="h-14 w-14 rounded-xl object-cover border border-white/15" />
                    <img v-else-if="ministry.logo_path && !form.remove_logo" :src="`/storage/${ministry.logo_path}`" class="h-14 w-14 rounded-xl object-cover border border-white/15" />
