    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}/annonces`" back-label="Annuler">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">Modifier l'annonce</h1>
            </div>
        </template>
                ...
                <section>
                    ...
                    <div class="space-y-4">
                        ...
                        <div v-if="announcement.attachment_path && !removeAttachment">
                            <p class="mb-1 text-sm font-medium text-white/80">Pièce jointe actuelle</p>
                            <div class="flex items-center justify-between rounded-xl border border-white/15 bg-white/5 px-3.5 py-2.5 text-sm">
                                <a :href="`/storage/${announcement.attachment_path}`" target="_blank" class="text-white/70 underline underline-offset-2 hover:text-gold-soft">
                                    {{ announcement.attachment_original_name }}
                                </a>
                                <button type="button" @click="toggleRemoveAttachment" class="text-xs font-medium text-rose-400 hover:underline">Retirer</button>
                            </div>
                        </div>
                        <div v-else-if="removeAttachment" class="rounded-xl border border-white/15 px-3.5 py-2.5 text-sm text-white/40">
                            La pièce jointe sera retirée à l'enregistrement.
                            <button type="button" @click="toggleRemoveAttachment" class="ml-2 text-xs font-medium text-white/70 hover:underline">Annuler</button>
                        </div>
                        <div>
