    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">Annonces</h1>
                <p class="text-sm text-white/55 mt-2 max-w-2xl">Diffusées depuis {{ orgUnit.name }} et tous les niveaux au-dessus.</p>
            </div>
        </template>
        ...
        <div class="max-w-3xl mx-auto space-y-8">
            ...
            <div v-if="announcements.length === 0" class="glass-panel flex flex-col items-center rounded-3xl px-8 py-14 text-center animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <p class="mt-4 text-sm font-medium text-white/70">Aucune annonce pour l'instant.</p>
            </div>
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <span v-if="announcement.important" class="rounded-full bg-gold/15 px-2 py-0.5 text-xs font-medium text-gold-soft">Important</span>
                                ...
                                <p class="font-semibold text-white text-[15px]">{{ announcement.title }}</p>
                            </div>
                            ...
                            <p v-if="announcement.body" class="mt-3 whitespace-pre-line text-sm text-white/70">{{ announcement.body }}</p>
                            <a
                                v-if="announcement.attachment_path"
                                :href="attachmentUrl(announcement)"
                                target="_blank"
                                class="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-white/70 underline underline-offset-2 hover:text-gold-soft"
                            >
                                Pièce jointe : {{ announcement.attachment_original_name }}
                            </a>
                            <p v-if="canManage && announcement.important" class="mt-3 text-xs text-white/35">
                        ...
                        <div class="flex flex-shrink-0 flex-col items-end gap-2">
                            <button
                                ...
                                class="rounded-lg glass-panel-light px-3 py-1.5 text-xs font-medium text-white/70 transition hover:text-white"
                            >
                                Marquer comme lu
                            </button>
                            <Link
                                ...
                                class="rounded-lg px-3 py-1.5 text-xs font-medium text-white/50 transition hover:bg-white/10 hover:text-white"
                            >
                                Modifier
                            </Link>
                        </div>
