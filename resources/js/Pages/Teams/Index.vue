    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-5xl font-bold text-graphite mt-2">{{ teams.length }} équipe{{ teams.length > 1 ? 's' : '' }}</h1>
        ...
        <div class="space-y-8">
            ...
            <div v-if="teams.length === 0" class="glass-panel flex flex-col items-center rounded-3xl px-8 py-14 text-center animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <p class="mt-4 text-sm font-medium text-graphite/80">Aucune équipe créée pour le moment.</p>
                <p class="mt-1 text-sm text-graphite/62">Crée une première équipe pour organiser le service.</p>
            </div>
                ...
                >
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-3">
                            <p class="truncate font-semibold text-graphite text-[15px]">{{ team.name }}</p>
                        ...
                        <p v-if="team.description" class="mt-1 text-sm text-graphite/65">{{ team.description }}</p>
                    </div>
