 ...
 * style v3 "Vitrail". Meme logique de conjoint facultatif (membre ou nom
 * libre) que l'ecran de creation.
 */
...
const form = useForm({
    type: props.sacrement.type,
    member_id: props.sacrement.member_id,
    spouse_member_id: props.sacrement.spouse_member_id ?? '',
    spouse_name: props.sacrement.spouse_name ?? '',
    event_date: props.sacrement.event_date,
    officiant: props.sacrement.officiant ?? '',
    location: props.sacrement.location ?? '',
    notes: props.sacrement.notes ?? '',
})
...
function submit() {
    form.put(`/org-units/${props.orgUnit.id}/sacrements/${props.sacrement.id}`)
}

function destroy() {
    if (confirm('Retirer ce sacrement ? Cette action est irréversible.')) {
        router.delete(`/org-units/${props.orgUnit.id}/sacrements/${props.sacrement.id}`)
    }
}
</script>
    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}/sacrements`" back-label="Annuler">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">Modifier le sacrement</h1>
            </div>
        </template>
                ...
                <section class="border-t border-white/10 pt-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input v-model="form.officiant" type="text" class="w-full bg-white/5 border border-white/15 text-white rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60" />
                        </div>
                        <div>
