    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">{{ sacraments.length }} sacrement{{ sacraments.length > 1 ? 's' : '' }}</h1>
                <p class="text-sm text-white/55 mt-2 max-w-2xl">Baptêmes et mariages enregistrés individuellement, pour chaque membre.</p>
            </div>
        </template>
        ...
        <div class="space-y-8">
            ...
            <div v-if="sacraments.length === 0" class="glass-panel flex flex-col items-center rounded-3xl px-8 py-14 text-center animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <p class="mt-4 text-sm font-medium text-white/70">Aucun sacrement enregistré pour le moment.</p>
                <p class="mt-1 text-sm text-white/40">Enregistre le premier baptême ou mariage pour garder trace de la vie des membres.</p>
            </div>
            ...
            <div v-else class="space-y-3">
                ...
                >
                    <div class="flex w-14 flex-shrink-0 flex-col items-center justify-center rounded-xl bg-white/5 border border-white/10 py-2">
                        <span class="text-lg font-bold leading-none text-white">{{ dayNumber(s.event_date) }}</span>
                        <span class="mt-1 text-[10px] font-semibold uppercase tracking-wide text-white/40">{{ monthAbbrev(s.event_date) }}</span>
                    </div>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-3">
                            <p class="truncate font-semibold text-white text-[15px]">
                                {{ memberName(s.member) }}
                                <span v-if="s.type === 'mariage'" class="text-white/45 font-normal">
                        ...
                        <p v-if="s.notes" class="mt-2 text-sm text-white/60">{{ s.notes }}</p>
                    </div>
