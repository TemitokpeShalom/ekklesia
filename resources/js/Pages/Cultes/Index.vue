...
function statusClass(status) {
    ...
    return 'bg-gold/15 text-gold-soft'
}
    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">{{ cultes.length }} culte{{ cultes.length > 1 ? 's' : '' }}</h1>
                <p class="text-sm text-white/55 mt-2 max-w-2xl">Historique des cultes, messages prêchés et assistance.</p>
            </div>
        </template>
        ...
        <div class="space-y-8">
            ...
            <div v-if="cultes.length === 0" class="glass-panel flex flex-col items-center rounded-3xl px-8 py-14 text-center animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <p class="mt-4 text-sm font-medium text-white/70">Aucun culte enregistré pour le moment.</p>
                <p class="mt-1 text-sm text-white/40">Ajoute le premier culte pour suivre les messages et l'assistance.</p>
            </div>
            ...
            <div v-else class="space-y-3">
                <div
                    ...
                    :key="culte.id"
                    class="flex items-start gap-4 glass-panel rounded-2xl p-5 hover:border-white/20 transition-all duration-300 animate-[fadeInUp_0.5s_ease-out_both]"
                    :style="{ animationDelay: `${Math.min(i, 10) * 40}ms` }"
                >
                    <div class="flex w-14 flex-shrink-0 flex-col items-center justify-center rounded-xl bg-white/5 border border-white/10 py-2">
                        <span class="text-lg font-bold leading-none text-white">{{ dayNumber(culte.service_date) }}</span>
                        <span class="mt-1 text-[10px] font-semibold uppercase tracking-wide text-white/40">{{ monthAbbrev(culte.service_date) }}</span>
                    </div>

                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-3">
                            <p class="truncate font-semibold text-white text-[15px]">{{ culte.title }}</p>
                        ...
                        <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-white/45">
                            <span v-if="culte.attendance_adults !== null || culte.attendance_children !== null" class="inline-flex items-center gap-1.5">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4 text-white/35">
