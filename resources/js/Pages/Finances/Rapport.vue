            ...
            <div v-for="devise in devises" :key="devise.currency" class="space-y-6 animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="glass-panel rounded-2xl p-5">
                        ...
                        <p class="mt-2 text-2xl font-serif text-graphite">{{ formatAmount(devise.totalEncaissements, devise.currency) }}</p>
                    </div>
                    <div class="glass-panel rounded-2xl p-5">
                        <p class="text-xs font-semibold uppercase tracking-widest text-sanctuary-light">Total décaissements</p>
                        <p class="mt-2 text-2xl font-serif text-graphite">{{ formatAmount(devise.totalDecaissements, devise.currency) }}</p>
                    </div>
                    <div class="glass-panel rounded-2xl p-5">
                        <p class="text-xs font-semibold uppercase tracking-widest text-graphite/68">Solde de trésorerie</p>
                        <p class="mt-2 text-2xl font-serif text-graphite">{{ formatAmount(devise.solde, devise.currency) }}</p>
                    </div>
                </div>
