...
import { queueMouvement } from '@/offlineFinancesQueue'

/**
 * v3 "Vitrail" (2026-09-09) : migration de ce module vers la coquille
 * partagee AppLayout, sans aucun changement fonctionnel.
 *
 * Chantier "hors connexion", huitieme pierre (2026-09-19) : meme principe
 * que Cultes/Create.vue - aucun fichier dans ce formulaire, donc rien a
 * desactiver hors connexion (voir offlineFinancesQueue.js). Le compte
 * comptable reste verifie cote serveur au moment de l'envoi, comme en
 * ligne.
 */
...
function submit() {
    ...
    if (typeof navigator !== 'undefined' && navigator.onLine === false) {
        queueMouvement(props.orgUnit.id, { ...form.data() })
        form.reset()
    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}/finances`" back-label="Retour">
        ...
        <div class="max-w-2xl mx-auto">
            ...
            >
                ...
                <span>Mouvement enregistré sur cet appareil (pas de connexion pour le moment). Il sera envoyé automatiquement vers la plateforme dès que la connexion revient, sans rien faire de plus.</span>
            </div>
