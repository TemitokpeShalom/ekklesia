...
const badgeClasses = {
    nouveau_converti: 'bg-white/10 text-white/60',
    baptise: 'bg-sanctuary/15 text-sanctuary-light',
    en_formation: 'bg-gold/15 text-gold-soft',
    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">Croissance spirituelle des membres</h1>
                <p class="text-sm text-white/55 mt-2 max-w-2xl">L'étape la plus récente de chaque membre, du nouveau converti à l'envoi en service.</p>
            </div>
        </template>
        ...
        <div class="space-y-8">
            ...
            <div v-if="members.length === 0" class="glass-panel flex flex-col items-center rounded-3xl px-8 py-14 text-center animate-[fadeInUp_0.55s_ease-out_both]">
                ...
                <p class="mt-4 text-sm font-medium text-white/70">Aucun membre enregistré pour l'instant.</p>
                <p class="mt-1 text-sm text-white/40">Ajoute des membres avant de suivre leur parcours de disciple.</p>
            </div>
                ...
                >
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-3">
                            <p class="truncate font-semibold text-white text-[15px]">{{ member.first_name }} {{ member.last_name }}</p>
                            <span
                            ...
                            <span v-else class="flex-shrink-0 rounded-full px-3 py-1 text-xs font-medium bg-white/5 text-white/35">
                                Aucune étape enregistrée
                            </span>
                        </div>
                        <p v-if="member.latest_discipleship_stage" class="mt-1 text-sm text-white/45">
                    ...
                    <Link
                        ...
                        class="flex-shrink-0 self-center rounded-lg px-3 py-1.5 text-sm font-medium text-gold-soft transition hover:bg-white/10 hover:text-gold"
                    >
                        Enregistrer une étape
                    </Link>
                </div>
