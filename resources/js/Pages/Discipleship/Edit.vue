...
<template>
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}/parcours`" back-label="Annuler">
        <template #title>
            <div class="animate-[fadeInUp_0.5s_ease-out_both]">
                ...
                <h1 class="font-serif text-3xl text-white mt-2">Modifier l'étape</h1>
            </div>
        </template>
        ...
        <div class="max-w-2xl mx-auto">
            <form @submit.prevent="submit" class="space-y-8 glass-panel rounded-3xl p-6 md:p-7 animate-[fadeInUp_0.55s_ease-out_both]">
                <section>
                    <h2 class="text-xs font-semibold text-gold-soft/80 uppercase tracking-widest mb-4">Membre concerné</h2>
                ...
                <section class="border-t border-white/10 pt-6">
                    <h2 class="text-xs font-semibold text-gold-soft/80 uppercase tracking-widest mb-4">Étape franchie</h2>
                    <select v-model="form.stage" class="w-full bg-white/5 border border-white/15 text-white rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60">
                        <option v-for="(label, key) in stages" :key="key" :value="key" class="bg-night text-white">{{ label }}</option>
                    </select>
                    <p v-if="form.errors.stage" class="mt-1 text-sm text-rose-400">{{ form.errors.stage }}</p>
                </section>

                <section class="border-t border-white/10 pt-6">
                    <h2 class="text-xs font-semibold text-gold-soft/80 uppercase tracking-widest mb-4">Date</h2>
                    <input v-model="form.reached_at" type="date" class="w-full bg-white/5 border border-white/15 text-white rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60" />
                    <p v-if="form.errors.reached_at" class="mt-1 text-sm text-rose-400">{{ form.errors.reached_at }}</p>
                </section>

                <section class="border-t border-white/10 pt-6">
                    <h2 class="text-xs font-semibold text-gold-soft/80 uppercase tracking-widest mb-4">Notes</h2>
