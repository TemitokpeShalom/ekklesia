    ...
    <AppLayout :org-unit="orgUnit" :back-href="`/org-units/${orgUnit.id}`">
        ...
        <div class="max-w-lg mx-auto space-y-6">
            ...
            <form @submit.prevent="submit" class="space-y-4 glass-panel rounded-3xl p-6 md:p-7 animate-[fadeInUp_0.55s_ease-out_both]">
                <div>
                    <label class="mb-1 block text-sm font-medium text-graphite/87">Niveau de la nouvelle entité</label>
                    <select v-model="form.target_level_rank" required class="w-full bg-graphite/5 border border-graphite/15 text-graphite rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60">
                        <option value="" disabled class="bg-white text-graphite">Choisir un niveau</option>
                        <option v-for="opt in availableRanks" :key="opt.rank" :value="opt.rank" class="bg-white text-graphite">{{ opt.label }}</option>
                    </select>
                    <p v-if="form.errors.target_level_rank" class="mt-1 text-sm text-rose-600">{{ form.errors.target_level_rank }}</p>
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-graphite/87">Durée de validité (heures)</label>
                    <input v-model.number="form.valid_for_hours" type="number" min="1" max="720" class="w-full bg-graphite/5 border border-graphite/15 text-graphite rounded-xl px-3.5 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/60" />
                    <p class="mt-1 text-xs text-graphite/58">72 heures par défaut, 720 heures (30 jours) maximum.</p>
                    <p v-if="form.errors.valid_for_hours" class="mt-1 text-sm text-rose-600">{{ form.errors.valid_for_hours }}</p>
                </div>
